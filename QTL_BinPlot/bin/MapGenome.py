...
'''
def convert_MAP(infile, outfile, chr_start):
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if line[0:1].isdigit():
                ...
                pos = int(unit[0][2:]) + chr_start[chrs]
                if last_pos.has_key(chrs):
                    pos = (pos - last_pos[chrs])/2 + last_pos[chrs] 
                    last_pos[chrs] = pos
                else:
                    last_pos[chrs] = chr_start[chrs]
                    pos = (pos - last_pos[chrs])/2 + last_pos[chrs]
                    last_pos[chrs] = pos
...
def main():
    ...
    if not os.path.isfile(args.output):
        convert_MAP(args.input, args.output, chr_start)
